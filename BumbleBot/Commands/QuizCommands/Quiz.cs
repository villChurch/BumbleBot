...
namespace BumbleBot.Commands.QuizCommands
{
    ...
    public class Quiz : BaseCommandModule
    {
        ...
        public async Task AddQuestion(CommandContext ctx, [Description("Channel for poll to be posted in")] DiscordChannel channel)
        {
            ...
            var questionResponse = await interactivity.WaitForMessageAsync(
                x => x.Author == ctx.Message.Author && x.Channel == ctx.Channel, TimeSpan.FromMinutes(5)).ConfigureAwait(false);
            ...
            string question = questionResponse.Result.Content;

            var multipleChoiceMsg = await ctx.Channel.SendMessageAsync("Is this question multiple choice?").ConfigureAwait(false);
            ...
            multipleChoiceMsg && reaction.User == ctx.User && (reaction.Emoji == thumbsdown || reaction.Emoji == thumbsup),
            ...
            if (multipleChoiceResponse.Result.Emoji == thumbsup)
            {
                ...
                string answerOne = answerOneResponse.Result.Content;
                ...
                string answerTwo = answerTwoResponse.Result.Content;
                ...
                string answerThree = answerThreeResponse.Result.Content;

                DiscordEmoji a = DiscordEmoji.FromName(ctx.Client, ":regional_indicator_a:");
                DiscordEmoji b = DiscordEmoji.FromName(ctx.Client, ":regional_indicator_b:");
                DiscordEmoji c = DiscordEmoji.FromName(ctx.Client, ":regional_indicator_c:");
                var correctAnswerMsg = await ctx.Channel.SendMessageAsync("Please react A B or C depending on which is the correct answer.").ConfigureAwait(false);
                ...
                var correctAnswerRespone = await interactivity.WaitForReactionAsync(x => x.Message == correctAnswerMsg && x.User == ctx.User
                && (x.Emoji == a || x.Emoji == b || x.Emoji == c), TimeSpan.FromMinutes(5)).ConfigureAwait(false);
                ...
                questionEmbed.AddField(DiscordEmoji.FromName(ctx.Client, ":regional_indicator_a:"), answerOne, false);
                questionEmbed.AddField(DiscordEmoji.FromName(ctx.Client, ":regional_indicator_b:"), answerTwo, false);
                questionEmbed.AddField(DiscordEmoji.FromName(ctx.Client, ":regional_indicator_c:"), answerThree, false);
                ...
                {
                    DiscordMember winner = await ctx.Guild.GetMemberAsync(correctAnswer.Result.User.Id).ConfigureAwait(false);
                    await channel.SendMessageAsync($"Congratulations {winner.DisplayName} you got the correct answer first").ConfigureAwait(false);
                }
            }
            else
            {
                ...
                string answer = answerResponse.Result.Content;
                ...
                && x.Content.Trim().ToLower() == answer.Trim().ToLower(), TimeSpan.FromMinutes(1)).ConfigureAwait(false);
                ...
                {
                    DiscordMember winner = await ctx.Guild.GetMemberAsync(singularResponse.Result.Author.Id);
